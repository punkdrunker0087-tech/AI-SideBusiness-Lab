...
### Pilot同等ルート（2026-07-11追記・実践との整合）

限界費用ゼロのデジタル商品（単一HTMLツール等）は「限定公開」に
コスト面の利点がないため、次の2条件を満たす**本公開**をPilot実施と
同等に扱ってよい。

1. 出品前に30日間の反証可能な判定基準（成立／修正／需要仮説棄却の
   分岐）をDecision Logに固定している（`knowledge/SOP/
   evaluation-judgment.md`原則4）
2. 無料版が併設され、需要シグナル（DL数）を売上と独立に計測できる

PJ-006（評価68点・Software）/PJ-007（同64点・Software）はこの
ルートで公開した実例である（判定は2026-08上旬）。
