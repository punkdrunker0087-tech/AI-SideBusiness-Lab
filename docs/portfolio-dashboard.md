...
| PJ-006 | クリエイター売上・KPI記録ツール（単一HTML） | CEO判断待ち | CEO | 要CEO判断（事業評価部案: Conditional Go、68点） | - | 2026-07-08 | CEO発案（Fable-5窓7/12までの新規ツール開発）。需要証拠あり（BOOTH「売上管理」タグ144商品・有料上限¥2,580）。評価68点はConditional Go上限、戦略適合性88は全案件中最高、Asset Score★5、Value Score74。条件: ①価格¥980以下＋フリーミアム②税務表現の法務確認③バックアップ機能必須。Go確定後、7/12までにMVP実装。将来チャネルとしてGoogle Play（PWA/TWA、実売検証後）を記録 |
