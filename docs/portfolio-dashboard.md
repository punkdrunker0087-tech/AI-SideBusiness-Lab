...
| PJ-009 | FX自動売買・個人研究ツール | 個人ツール（CEO自己運用・スコア対象外） | CEO（自己運用） | 対象外（事業パイプライン外・PJ-004同様） | - | 2026-07-15 | CEO発案。**売る＝金商法・信頼で強制No-Go**、CEO個人の研究ツールなら可（PJ-004型）と裁定。ガードレール: AIは口座認証情報を持たない/自動発注しない（実弾は人間・🔴）、AIは第1〜6段階（研究・バックテスト基盤）まで、リスク上限を先に固定（金額CEO確定待ち）、AI生成コードは破壊者で検証（先読みバグ・スプレッド・計算精度）。卒業ゲート非算入。`cases/PJ-009/intake.md`・`knowledge/Decision Log/2026-07-15-fx-tool-ruling.md` |
...
| 2026-07-14 | 事前登録した判定の自動発火を設定（CEO方針「承認不要はどんどん自動化」・プリモーテムの"判定形骸化"リスク対策）: 7/25プリモーテム・チャネル転換判定、8/8 PJ-006判定、8/9 PJ-007判定を各one-shot Routineで自動実行（判定分析は自動、最終決定はCEO）。稼働中の全自動化を`docs/automation-registry.md`に一覧化（増殖防止・キルスイッチ）。自動化の大原則「材料づくりは自動・経営判断は人間」を明文化 |
