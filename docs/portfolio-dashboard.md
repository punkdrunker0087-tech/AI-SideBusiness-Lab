...
| PJ-008 | AI動画編集システム（実測ファースト検証） | 調査中（CEOの実測待ち・発掘フェーズのためアクティブ枠外） | CEO（実測）＋制作部（パイプライン整備） | 中 | - | 2026-07-12 | CEO発案「AI動画編集代行が鉄板ジャンル？」に対し工程0で前提の穴4件（情報源バイアス・経営原則8との衝突・¥5,000は売上であって利益ではない・実行環境にffmpeg無し）を特定。CEO決定は実測ファースト: 3工程パイプライン（無音カット→whisper字幕→焼き込み、`cases/PJ-008/pipeline/`）をCEOのPCで動かし人間時間を実測。判定基準は事前固定（45分以内=受注テスト1件／45〜90分=商品化ルート検証／90分超=棚上げ）。スクリプトは未検証（初回実行がテスト） |
