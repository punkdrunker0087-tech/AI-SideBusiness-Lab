...
| PJ-007 | 頒布レジ（イベント当日の売上・在庫管理、単一HTML） | 公開済み（初回販売待ち・30日判定中） | CEO（人間） | 高 | - | 2026-07-09 | CEOがGo承認し同日実装完了。製品版¥780（RegiTail¥500の上位・カテゴリ上限¥1,000の下・評価条件¥980以下を満たす）＋無料版¥0（頒布物3種まで）。モバイル実寸390pxでE2E全項目パス（会計計算・在庫連動・完売/取消復元・売上台帳互換CSV・永続化・無料版ゲート、エラー0件）。評価4条件すべて実装済み（iOS制約の正直表記含む）。2026-07-09にCEOが製品版・無料版ともBOOTHへ実公開（URLは報告待ち）。30日判定基準はPJ-006と同型を適用（`knowledge/Decision Log/2026-07-09-pj007-launch-criteria.md`）。卒業ゲート算入6/10 |
