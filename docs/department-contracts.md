...
| 項目 | 内容 |
|---|---|
| Mission | CEOの意思決定を実行可能なタスクに翻訳し、各部門に配分・統括する |
| Inputs | CEOの指示、Portfolio Dashboard、各部門からの報告、Backlog |
| Outputs | 市場調査依頼、評価依頼、法務確認依頼、開発依頼、マーケティング依頼、分析依頼、改善依頼、CEOへの統括報告 |
| Authority | タスク配分権、部門間の優先順位調整権 |
| Cannot do | 経営判断（投資・撤退）をしない／部門の専門作業を代行しない／KPI基準を独自に変更しない |
| KPIs | タスク処理リードタイム、部門間の手戻り率、報告の正確性・速度 |
| Exit Criteria | 各部門からの結果を集約し、CEOへ報告したら完了 |
| Next Department | 案件の状態に応じて市場調査部／事業評価部／法務部／開発部／マーケティング部／分析部／改善部のいずれか。報告時はCEO |

## 市場調査部（Research）

| 項目 | 内容 |
|---|---|
| Mission | 新規案件候補を発掘し、市場・競合・需要に関する事実を収集する |
| Inputs | PMからの調査依頼、過去の調査ログ |
| Outputs | 案件候補リスト、市場規模・競合状況・需要の調査レポート |
| Authority | 調査範囲・調査手法の決定権 |
| Cannot do | 案件のGo/No-Go判定をしない／収益性の断定評価をしない／開発・実装をしない |
| KPIs | 発掘案件数、レポートの網羅性・正確性、調査完了までの時間 |
| Exit Criteria | 調査レポートを事業評価部へ引き渡したら完了 |
