...
| Outputs | 販売ページ・X投稿・note記事・商品説明・SEOタイトル・サムネイル指示書（最低6点）、チャネルごとの公開状態、公開済み商品の継続改善仮説（週次、Marketing.md「継続改善仮説」参照） |
| Authority | 集客施策・チャネル選定・価格提示（PM承認の範囲内）の決定権 |
| Cannot do | 製品仕様を変更しない／収益性の最終評価をしない／法務判断をしない／A/Bテスト・改善仮説の採否を自ら判定しない |
| KPIs | 販売ページ公開数、CTR、CVR、初回販売日、売上、レビュー数、週次改善仮説提示件数 |
...
| Inputs | 分析部のKPIレポート、過去の改善ログ、`knowledge/Customer Voice/`のお客様の声 |
