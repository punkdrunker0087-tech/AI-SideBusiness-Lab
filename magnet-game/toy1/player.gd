...
# プレイヤー1人ぶんの移動・ジャンプ・磁力の受け取り・復帰・ボタン操作。
...
var use_keyboard_wasd := false  # 1PはWASD+Space+Eでも操作できる
var use_keyboard_arrows := false# 2Pは矢印キー+Enter+/でも操作できる
var polarity := 1               # +1 / -1
...
func interact_just_pressed() -> bool:
	var now := _interact_held()
	var edge := now and not _interact_prev
	_interact_prev = now
	return edge
...
func _interact_held() -> bool:
	if Input.is_joy_button_pressed(device, JOY_BUTTON_X):
		return true
	if use_keyboard_wasd and Input.is_key_pressed(KEY_E):
		return true
	if use_keyboard_arrows and Input.is_key_pressed(KEY_SLASH):
		return true
	return false
