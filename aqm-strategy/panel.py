...
def _fix_unadjusted_splits(close: pd.Series, jump_threshold: float = 2.5,
                          confirm_days: int = 5) -> pd.Series:
    """adjcloseが遡及調整し損ねた株式分割・併合を検出し、遡及調整する。

    実装時、1306.T(TOPIX連動ETF)が2015年1月に実施した10:1分割で
    adjcloseが2014-12-30(986円)→2015-01-05(98円)と、遡及調整されないまま
    段差を残していることを発見した（`_sanitize`が対象とする一時的な
    1日グリッチとは異なり、ジャンプ後も新水準がずっと続く「本物の」
    分割/併合であるため、5日ローリング中央値ベースの`_sanitize`では
    検出できない）。1日で`jump_threshold`倍以上/以下になり、かつその後
    `confirm_days`日その新水準が続く（=一時的なグリッチで元に戻るのでは
    ない）場合のみ、ジャンプ以前の全データをその比率で遡及調整する。
    """
    s = close.copy()
    ...
    for i in range(1, n):
        ...
        if prev <= 0 or cur <= 0 or np.isnan(prev) or np.isnan(cur):
            continue
        ratio = cur / prev
        if ratio > jump_threshold or ratio < 1.0 / jump_threshold:
            window = values[i:min(i + confirm_days, n)]
            window = window[~np.isnan(window)]
            if len(window) == 0:
                continue
            if abs(np.median(window) / cur - 1) < 0.15:
                values[:i] *= ratio
...
def _fetch_one(symbol: str, range_: str, use_cache: bool) -> pd.DataFrame:
    ...
    # 先に1日グリッチ除去(_sanitize)を行ってから永続的な分割検出
    # (_fix_unadjusted_splits)を行う。逆順だと、グリッチが元の水準へ
    # 戻る"回復"の一歩を"本物の持続的な分割"と誤認し、履歴全体を誤って
    # スケーリングしてしまうバグがあった（実装時に発見・修正）。
    df["close"] = _sanitize(df["close"])
    df["close"] = _fix_unadjusted_splits(df["close"])
