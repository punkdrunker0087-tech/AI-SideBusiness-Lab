...
- **`CONSTITUTION.md`** — LIMIT LAB憲法。Purpose（存在意義）・Mission・
  ...
  すべての判断の最上位規範
- **`docs/architecture-governance.md`** — 「なぜ」に加えて「今やるべきか、
  他で代替できないか」を問う運用ゲート（MVOS・Architecture Change Gate・
  Reuse First・Feature Freeze・Build Less, Learn More）
...
- **`docs/human-desire-framework.md`** — 人間理解のフレームワーク
  （6段階の欲求レベル、Primary/Secondary Desire等の分析ルール）。
  「欲求を作る」のではなく理解し誠実に伝えるための道具。Values
  「信頼と信用」「先義後利」への貢献
...
- **`knowledge/Institutional Memory/`** — 現場レベルで再利用できる知見
  ...
  Values「継続的な改善」への貢献
- **`knowledge/SOP/`** — 標準作業手順書。高性能モデルが確立した手順を、
  後続のどのAIモデルでも再現できる具体性で記録する（例: 単一HTML
  ツール工場・価格決定・事業評価の採点・販売ページ監査の各SOP。
  一覧は`knowledge/SOP/README.md`）
